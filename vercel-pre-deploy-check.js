...
try {
  if (!fs.existsSync('vercel.json')) {
    ...
    // Criar um vercel.json básico
    console.log('Criando um arquivo vercel.json básico...');
    const vercelConfig = {
      "version": 2,
    ...
    console.log('✅ vercel.json criado com configurações básicas');
  } else {
...
try {
  if (fs.existsSync('.vercelignore')) {
    ...
    // Criar um .vercelignore básico
    console.log('Criando um arquivo .vercelignore básico...');
...
# Arquivos de configuração específicos para local
.replit
.replit.nix
replit.nix
`;
    fs.writeFileSync('.vercelignore', vercelIgnoreContent.trim());
    console.log('✅ .vercelignore criado com regras básicas');
  }
} catch (error) {
